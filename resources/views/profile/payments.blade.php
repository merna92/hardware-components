<x-layout.layout :title="__('My Payment Options') . ' - ' . __('Dashboard')">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-3">
                <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
                    ...
                    <ul class="list-unstyled d-flex flex-column gap-2 mb-4 fs-6">
                        ...
                        <li><a href="{{ route('payments.index') }}" class="text-decoration-none fw-semibold text-danger">{{ __('My Payment Options') }}</a></li>
                    </ul>
            ...
            <div class="col-lg-9">
                <div class="card border-0 shadow-sm rounded-4 p-4 p-md-5 bg-white">
                    ...
                    <h4 class="fw-bold text-danger mb-1">{{ __('My Payment Options') }}</h4>
                    <p class="text-muted small mb-4">{{ __('Manage your wallet balances and save your verified payment accounts.') }}</p>

                    <!-- Store Wallet Balance -->
                    <div class="bg-dark text-white rounded-4 p-4 mb-5 position-relative overflow-hidden">
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                <p class="mb-1 small text-white-50"><i class="bi bi-wallet2 me-1"></i> {{ __('Store Wallet Balance') }}</p>
                                <h2 class="fw-bold mb-1">$0.00 <small class="text-white-50 fs-6">USD / EGP</small></h2>
                                <p class="small text-white-50 mb-0"><i class="bi bi-info-circle me-1"></i> {{ __('Refunds from cancelled orders or returns are automatically credited here for instant checkout.') }}</p>
                            </div>
                            <span class="badge bg-success px-3 py-2 rounded-pill fw-semibold">{{ __('Active Balance') }}</span>
                        </div>
                    </div>

                    <!-- Add Verified Payment Option -->
                    <h5 class="fw-bold text-dark mb-1"><i class="bi bi-shield-check text-danger me-1"></i> {{ __('Add Verified Payment Option') }}</h5>
                    <p class="text-muted small mb-4">{{ __('Select your payment method below to configure account security rules:') }}</p>
                    ...
                    <div class="collapse mb-4" id="paymentFormCollapse">
                        <form action="{{ route('payments.store') }}" method="POST" class="p-4 border rounded-4 bg-light">
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-semibold" id="payment_label">{{ __('Account Details') }}</label>
                                <input type="text" name="account_details" class="form-control bg-white" placeholder="{{ __('Enter your account number or details') }}" required>
                            </div>
                            <button type="submit" class="btn btn-danger px-4 rounded-3 fw-semibold">{{ __('Save Payment Method') }}</button>
                        </form>
                    </div>

                    <!-- Saved Payment Accounts -->
                    <h5 class="fw-bold text-dark mb-3"><i class="bi bi-credit-card-2-back text-danger me-1"></i> {{ __('Saved Payment Accounts') }}</h5>

                    @if($methods->isEmpty())
                        <div class="bg-light rounded-4 p-4 text-center text-muted">
                            {{ __('No verified accounts saved yet. Click any icon above to configure your gateway.') }}
                        </div>
                    @else
                        <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    <div class="p-3 border rounded-4 bg-white d-flex justify-content-between align-items-center">
                                        ...
                                        <form action="{{ route('payments.destroy', $method->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-outline-danger btn-sm rounded-pill">{{ __('Remove') }}</button>
                                        </form>
                                    </div>
                                </div>
                            @endforeach
