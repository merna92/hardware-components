<x-layout.layout :title="__('My Wishlist') . ' - ' . __('Dashboard')">
    <div class="container py-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h3 class="fw-bold text-dark mb-0">{{ __('My Wishlist') }} ({{ count($wishlists) }})</h3>
            <a href="{{ route('catalog.index') }}" class="btn btn-outline-dark rounded-pill px-4">{{ __('Continue Shopping') }}</a>
        </div>
        ...
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-4">
                    ...
                    <div class="col">
                        <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden position-relative bg-white">
                            <form action="{{ route('wishlist.destroy', $wishlist->id) }}" method="POST" class="position-absolute top-0 end-0 m-3 z-3">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-light btn-sm rounded-circle shadow-xs" title="{{ __('Remove') }}">
            ...
            @empty
                <div class="col-12 text-center py-5">
                    ...
                    <p class="mt-3 text-muted fs-5">{{ __('Your wishlist is currently empty.') }}</p>
                    <a href="{{ route('catalog.index') }}" class="btn btn-danger px-4 rounded-pill">{{ __('Explore Products') }}</a>
                </div>
            @endforelse
        </div>
    </div>
